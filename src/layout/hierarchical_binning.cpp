...
size_t hierarchical_binning::add_lower_level(data_store & libf_data) const
{
    ...
    else
    {
        ...
        // The empty-bin adjustment can, for some (tmax, empty_bin_fraction, number_of_user_bins) combinations,
        // round down to fewer technical bins than there are user bins. simple_binning requires
        // number_of_technical_bins >= number_of_user_bins, so never reserve fewer than that.
        size_t const number_of_technical_bins =
            std::max(number_of_user_bins,
